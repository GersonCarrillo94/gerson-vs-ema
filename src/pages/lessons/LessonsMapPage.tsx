...
const LEVEL_SECTIONS = [
  { key: 'basic',        range: '1–12',   color: 'text-blue-600',    bg: 'bg-blue-50' },
  { key: 'intermediate', range: '13–24',  color: 'text-emerald-600', bg: 'bg-emerald-50' },
  { key: 'advanced',     range: '25–36',  color: 'text-amber-600',   bg: 'bg-amber-50' },
] as const;
...
export function LessonsMapPage() {
  ...
  const langLabel = user?.language_learning === 'spanish'
    ? t('lessons.languageSpanish')
    : t('lessons.languageEnglish');
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 animate-fade-in">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          {t('lessons.title', { language: langLabel })}
        </h1>
        <p className="mt-1 text-sm text-gray-500">
          {t('lessons.completed', { count: completed })}
        </p>
      ...
      {LEVEL_SECTIONS.map(({ key, range, color, bg }) => {
        ...
        const label = t(`lessons.levels.${key}`);
          ...
          <section key={key} className={`mb-8 rounded-2xl p-4 ${bg}`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <span className="text-xs text-gray-500">{t('lessons.sublevels', { range })}</span>
              </div>
              <span className={`text-sm font-semibold ${color}`}>{sectionCompleted}/12</span>
            </div>
      ...
      <div className="flex items-center gap-4 justify-center mt-2 text-xs text-gray-500">
        <span className="flex items-center gap-1">
          <span className="inline-block w-4 h-4 rounded bg-blue-500" /> {t('lessons.legend.available')}
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block w-4 h-4 rounded bg-blue-100 border border-blue-300" /> {t('lessons.legend.completed')}
        </span>
        <span className="flex items-center gap-1">
          <span className="inline-block w-4 h-4 rounded bg-gray-100" /> 🔒 {t('lessons.legend.locked')}
        </span>
      </div>
    </div>
  );
